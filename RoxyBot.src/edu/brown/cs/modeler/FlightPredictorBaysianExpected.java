 ...
 * </legal>
 *
 * FlightPredictorBaysianExpected
 *
...
public class FlightPredictorBaysianExpected extends FlightPredictor {
	...
	public FlightPredictorBaysianExpected() {
		init();
	}
	...
	protected float getPrice (int auction, int option, int decisec, int curPrice) {
		switch (option) {
		case WITHIN_1MIN:
			return getExpectedMinPrice (auction, TIME_WITHIN_1MIN, curPrice, decisec);
		case AFTER_1MIN:
			return getExpectedMinPrice (auction, TIME_AFTER_1MIN, curPrice, decisec);
		case AFTER_MIN:
			return Math.min(
					getExpectedMinPrice (auction, TIME_WITHIN_1MIN, curPrice, decisec), 
					getExpectedMinPrice (auction, TIME_AFTER_1MIN, curPrice, decisec));
		case HIST_MIN:
	...
	public FlightPredictor clone() {
		FlightPredictor ret = new FlightPredictorBaysianExpected();
		return ret;
	}
}
