 ...
 * Updated : $Date: 2007-05-16 20:03:32 $
 *	     $Revision: 1.2 $
...
public class TACAgent implements Task, TACMessageReceiver {
  ...
  public final static String VERSION = "Beta 8";
  ...
  public TACAgent(AgentImpl agent, String host, int port,
    ...
    if (!connection.isConnected()) {
      fatalError("could not connect to server " + host + " at port "
		 + port);
    }
  }
...
//			 log.info("requesting quote for " + auctionID);
	...
	log.warning("still awaiting quote for auction " + auction
				...
				+ ") after "
				+ ((currentTime - pendingQuotes[auction]) / 1000)
				+ " sec");
			} else {
	...
	log.warning("WARNING: transaction timeout!!! (resending)");
	log.warning("WARNING: Please contact tac-dev@sics.se and report this!!!");
			}
		}
	}
